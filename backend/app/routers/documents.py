...
from app.core import acces_dossier
...
async def _assert_access(db: AsyncSession, dossier_id: str, user: User, *, ecriture: bool = False):
    """Accès aux pièces d'un dossier — règle commune (`core/acces_dossier`).

    La consultation des pièces est en lecture : sans quoi l'auteur d'un dossier
    routé ne pourrait plus vérifier ce qu'il y a lui-même déposé. C'est aussi
    l'appel que joue « Soumettre pour analyse » avant la transition — refusé, il
    affichait « Accès refusé » à la place du décompte des pièces manquantes.
    """
...
async def upload_document(
    ...
    await _assert_access(db, dossier_id, current_user, ecriture=True)
