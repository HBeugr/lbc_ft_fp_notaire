<template>
  <AppLayout>
    <div class="p-6 space-y-5">
      <!-- Header -->
      <div class="flex items-center gap-3">
        <RouterLink to="/dossiers" class="text-gray-400 hover:text-gray-600">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </RouterLink>
        <div>
          <h1 class="text-xl font-semibold text-gray-900">KYC — Personne Morale</h1>
          <p class="text-sm text-gray-500">Dossier {{ dossierRef || dossierId }}</p>
        </div>
        <div v-if="scoreResult" class="ml-auto">
          <span :class="classBadge(scoreResult.classification)" class="px-3 py-1 rounded-full text-sm font-medium">
            {{ scoreResult.classification }}{{ scoreResult.force_par_trigger ? ' ⚡' : '' }} — {{ scoreResult.score }}/20
          </span>
        </div>
      </div>

      <!-- Stepper tabs -->
      <div class="flex border-b border-gray-200 overflow-x-auto">
        <button
          v-for="(step, i) in steps"
          :key="i"
          @click="currentStep = i"
          :class="[
            'flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px whitespace-nowrap',
            currentStep === i ? 'text-[#1a2e4a] border-[#1a2e4a]' : 'text-gray-500 border-transparent hover:text-gray-700',
          ]"
        >
          <span :class="['w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0', currentStep === i ? 'bg-[#1a2e4a] text-white' : 'bg-gray-200 text-gray-600']">{{ i + 1 }}</span>
          {{ step }}
        </button>
      </div>

      <!-- Messages -->
      <div v-if="saveError" class="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg">{{ saveError }}</div>
      <div v-if="saveOk" class="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-lg">KYC enregistré avec succès.</div>

      <!-- Form body -->
      <div class="bg-white rounded-xl border border-gray-200 p-6">

        <!-- 0 — Identité PM -->
        <div v-show="currentStep === 0" class="space-y-5">
          <h2 class="section-title">Identification de la Personne Morale</h2>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label class="label">Type de relation</label>
              <select v-model="form.relation_type" class="input">
                <option value="initiale">Entrée en relation initiale</option>
                <option value="actualisation">Actualisation</option>
              </select>
            </div>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
              <label class="label">Dénomination sociale <span class="req">*</span></label>
              <input v-model="form.denomination_sociale" required class="input" placeholder="Raison sociale complète" />
            </div>
            <div>
              <label class="label">Forme juridique</label>
              <select v-model="form.forme_juridique" class="input">
                <option value="">—</option>
                <option>SARL</option><option>SA</option><option>SAS</option><option>SNC</option>
                <option>GIE</option><option>Association</option><option>Fondation</option>
                <option>Coopérative</option><option>Autre</option>
              </select>
            </div>
            <div>
              <label class="label">Nom du représentant légal</label>
              <input v-model="form.nom_representant_legal" class="input" />
            </div>
            <div>
              <label class="label">N° RCCM</label>
              <input v-model="form.numero_rccm" class="input" placeholder="CI-ABJ-…" />
            </div>
            <div>
              <label class="label">N° Compte Contribuable / Identité Fiscale</label>
              <input v-model="form.numero_contribuable" class="input" />
            </div>
            <div class="md:col-span-2">
              <label class="label">Libellé de l'activité</label>
              <input v-model="form.libelle_activite" class="input" placeholder="Description précise de l'activité principale" />
            </div>
          </div>
        </div>

        <!-- 1 — Coordonnées -->
        <div v-show="currentStep === 1" class="space-y-5">
          <h2 class="section-title">Coordonnées</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
              <label class="label">Adresse du siège social</label>
              <input v-model="form.adresse" class="input" placeholder="Quartier, rue, immeuble, ville…" />
            </div>
            <div><label class="label">Téléphone</label><input v-model="form.telephone" class="input" placeholder="+225…" /></div>
            <div><label class="label">WhatsApp</label><input v-model="form.whatsapp" class="input" placeholder="+225…" /></div>
            <div><label class="label">Email</label><input v-model="form.email" type="email" class="input" /></div>
          </div>
        </div>

        <!-- 2 — Informations PM étendues -->
        <div v-show="currentStep === 2" class="space-y-5">
          <h2 class="section-title">Informations PM Complémentaires</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="label">Domaine d'activité</label>
              <select v-model="form.infos_pm.domaine_activite" class="input">
                <option value="">—</option>
                <option>Immobilier</option><option>Commerce</option><option>Services</option>
                <option>Industrie</option><option>Finance</option><option>Agriculture</option>
                <option>BTP</option><option>Technologie</option><option>Autre</option>
              </select>
            </div>
            <div>
              <label class="label">Nature de la PM</label>
              <select v-model="form.infos_pm.nature_pm" class="input">
                <option value="">—</option>
                <option>Société commerciale</option><option>Holding</option>
                <option>Filiale</option><option>Société offshore</option>
                <option>ONG / Association</option><option>Coopérative</option><option>Autre</option>
              </select>
            </div>
            <div>
              <label class="label">Ancienneté de la société</label>
              <select v-model="form.anciennete_pro" class="input">
                <option value="">—</option>
                <option value="moins_1_an">Moins de 1 an</option>
                <option value="1_a_10_ans">1 à 10 ans</option>
                <option value="plus_10_ans">Plus de 10 ans</option>
              </select>
            </div>
            <div class="flex items-center gap-6 pt-5">
              <label class="flex items-center gap-2 cursor-pointer">
                <input v-model="form.infos_pm.cotee" type="checkbox" class="w-4 h-4 rounded" />
                <span class="text-sm text-gray-700">Société cotée en bourse</span>
              </label>
            </div>
            <div v-if="form.infos_pm.cotee">
              <label class="label">Marché réglementé</label>
              <input v-model="form.infos_pm.marche_reglemente" class="input" placeholder="BRVM, NYSE…" />
            </div>
          </div>
        </div>

        <!-- 3 — Mandataire -->
        <div v-show="currentStep === 3" class="space-y-5">
          <h2 class="section-title">Représentant Légal / Mandataire</h2>
          <div class="border border-gray-200 rounded-lg p-4 space-y-3">
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="hasMandataire" type="checkbox" class="w-4 h-4 rounded" />
              <span class="text-sm font-medium text-gray-700">Le dossier est géré par un mandataire (autre que le représentant légal)</span>
            </label>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              <div><label class="label">Prénom & Nom <span v-if="!hasMandataire" class="text-gray-400">(Représentant légal)</span></label><input v-model="form.mandataire.prenom_nom" class="input" /></div>
              <div><label class="label">Fonction</label><input v-model="form.mandataire.fonction" class="input" placeholder="DG, PDG, Mandataire…" /></div>
              <div>
                <label class="label">Type de pièce</label>
                <select v-model="form.mandataire.type_piece" class="input">
                  <option value="">—</option><option value="CNI">CNI</option><option value="Passeport">Passeport</option>
                </select>
              </div>
              <div><label class="label">N° de pièce</label><input v-model="form.mandataire.numero_piece" class="input" /></div>
              <div><label class="label">Date de naissance</label><input v-model="form.mandataire.date_naissance" type="date" class="input" /></div>
              <div><label class="label">Nationalité</label><input v-model="form.mandataire.nationalite" class="input" /></div>
              <div><label class="label">Pays de résidence</label><input v-model="form.mandataire.pays_residence" class="input" /></div>
            </div>
          </div>
        </div>

        <!-- 4 — Bénéficiaires effectifs + Actionnaires -->
        <div v-show="currentStep === 4" class="space-y-5">
          <h2 class="section-title">Bénéficiaires Effectifs & Structure de Propriété</h2>

          <!-- BE ≥ 25% -->
          <div class="border border-gray-200 rounded-lg p-4 space-y-3">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="text-sm font-medium text-gray-700">Bénéficiaires Effectifs ≥ 25% <span class="text-xs text-gray-500">(Art. 12b)</span></h3>
                <p class="text-xs text-gray-400 mt-0.5">Personnes physiques détenant directement ou indirectement ≥ 25% du capital</p>
              </div>
              <button type="button" @click="addBE" class="text-xs text-[#1a2e4a] border border-[#1a2e4a] px-2 py-1 rounded hover:bg-[#1a2e4a]/5">+ Ajouter</button>
            </div>
            <p v-if="beList.length === 0" class="text-xs text-gray-400">Aucun bénéficiaire effectif ajouté.</p>
            <div v-for="(be, i) in beList" :key="'be'+i" class="grid grid-cols-1 md:grid-cols-3 gap-3 p-3 bg-gray-50 rounded-lg">
              <div><label class="label">Nom / Raison sociale <span class="req">*</span></label><input v-model="be.raison_sociale_nom" class="input" /></div>
              <div><label class="label">N° CNI / Passeport</label><input v-model="be.cni_passeport" class="input" /></div>
              <div><label class="label">% Participation</label><input v-model.number="be.pourcentage" type="number" min="0" max="100" step="0.01" class="input" /></div>
              <div><label class="label">Nationalité</label><input v-model="be.nationalite" class="input" /></div>
              <div><label class="label">Pays de résidence</label><input v-model="be.pays_residence" class="input" /></div>
              <div class="flex items-end"><button type="button" @click="beList.splice(i, 1)" class="text-xs text-red-500 hover:text-red-700 px-2 py-1">Supprimer</button></div>
            </div>
          </div>

          <!-- Actionnaires / Structure de propriété -->
          <div class="border border-gray-200 rounded-lg p-4 space-y-3">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="text-sm font-medium text-gray-700">Structure de Propriété</h3>
                <p class="text-xs text-gray-400 mt-0.5">Actionnaires / associés par ordre décroissant de participation</p>
              </div>
              <button type="button" @click="addActionnaire" class="text-xs text-[#1a2e4a] border border-[#1a2e4a] px-2 py-1 rounded hover:bg-[#1a2e4a]/5">+ Ajouter</button>
            </div>
            <p v-if="actList.length === 0" class="text-xs text-gray-400">Aucun actionnaire ajouté.</p>
            <div v-for="(act, i) in actList" :key="'act'+i" class="grid grid-cols-1 md:grid-cols-3 gap-3 p-3 bg-gray-50 rounded-lg">
              <div class="flex items-center gap-2">
                <span class="text-lg font-bold text-gray-300">#{{ i + 1 }}</span>
                <div class="flex-1"><label class="label">Nom / Raison sociale <span class="req">*</span></label><input v-model="act.raison_sociale_nom" class="input" /></div>
              </div>
              <div><label class="label">N° CNI / Passeport</label><input v-model="act.cni_passeport" class="input" /></div>
              <div><label class="label">% Part</label><input v-model.number="act.pourcentage" type="number" min="0" max="100" step="0.01" class="input" /></div>
              <div><label class="label">Pays de résidence</label><input v-model="act.pays_residence" class="input" /></div>
              <div class="flex items-end"><button type="button" @click="actList.splice(i, 1)" class="text-xs text-red-500 hover:text-red-700 px-2 py-1">Supprimer</button></div>
            </div>
          </div>
        </div>

        <!-- 5 — PPE + Description de l'opération -->
        <div v-show="currentStep === 5" class="space-y-5">
          <h2 class="section-title">PPE & Description de l'Opération</h2>

          <!-- PPE -->
          <div class="border rounded-lg p-4 space-y-3" :class="form.ppe_detectee ? 'border-orange-300 bg-orange-50' : 'border-gray-200'">
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.ppe_detectee" type="checkbox" class="w-4 h-4 rounded" />
              <span class="text-sm font-medium text-gray-700">PPE détectée parmi les dirigeants / bénéficiaires effectifs</span>
              <span class="text-xs text-orange-600 font-semibold">⚡ Trigger T1 — force ÉLEVÉ</span>
            </label>
            <div v-if="form.ppe_detectee">
              <label class="label">Détail PPE (nom, fonctions, pays, durée…)</label>
              <textarea v-model="form.ppe_detail" rows="2" class="input resize-none" />
            </div>
          </div>

          <!-- Opérations cochées -->
          <div>
            <p class="text-xs text-gray-500 mb-3">Nature(s) de l'opération :</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label v-for="op in operationOptions" :key="op.key"
                class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors"
                :class="form.operations_cochees[op.key] ? 'border-[#1a2e4a] bg-[#1a2e4a]/5' : 'border-gray-200'">
                <input v-model="form.operations_cochees[op.key]" type="checkbox" class="w-4 h-4 rounded text-[#1a2e4a]" />
                <span class="text-sm text-gray-700">{{ op.label }}</span>
              </label>
            </div>
            <div class="mt-3"><label class="label">Préciser si autre</label><input v-model="form.operations_cochees.autre_detail" class="input" /></div>
          </div>

          <div>
            <label class="label">Description détaillée de l'opération</label>
            <textarea v-model="form.description_operation" rows="4" class="input resize-none" placeholder="Objet précis de l'acte, parties, montants, conditions, répartitions…" />
          </div>
        </div>

        <!-- 6 — Origine fonds + Signature -->
        <div v-show="currentStep === 6" class="space-y-5">
          <h2 class="section-title">Origine des Fonds & Déclaration</h2>
          <p class="text-xs text-gray-500">La société déclare que les fonds engagés proviennent de :</p>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label v-for="of in fondOptions" :key="of.key"
              class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors"
              :class="form.origine_fonds[of.key] ? 'border-[#1a2e4a] bg-[#1a2e4a]/5' : 'border-gray-200'">
              <input v-model="form.origine_fonds[of.key]" type="checkbox" class="w-4 h-4 rounded text-[#1a2e4a]" />
              <span class="text-sm text-gray-700">{{ of.label }}</span>
            </label>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="label">Autres sources</label><input v-model="form.origine_fonds.autres" class="input" /></div>
            <div><label class="label">Pays de provenance (si hors CI)</label><input v-model="form.origine_fonds.pays_provenance" class="input" /></div>
          </div>
          <div>
            <label class="label">Date de signature</label>
            <input v-model="form.date_signature" type="date" class="input w-48" />
          </div>

          <!-- Score preview -->
          <div v-if="scoreResult" class="border rounded-lg p-4 space-y-2"
            :class="scoreResult.classification === 'ELEVE' ? 'border-red-200 bg-red-50' : scoreResult.classification === 'MOYEN' ? 'border-orange-200 bg-orange-50' : 'border-green-200 bg-green-50'">
            <div class="flex items-center gap-2 flex-wrap">
              <span :class="classBadge(scoreResult.classification)" class="px-2 py-0.5 rounded-full text-xs font-medium">{{ scoreResult.classification }}</span>
              <span class="text-sm font-medium text-gray-700">Score : {{ scoreResult.score }}/20</span>
              <span v-if="scoreResult.force_par_trigger" class="text-xs text-red-600 font-medium">⚡ Trigger {{ scoreResult.trigger_principal }}</span>
            </div>
            <div class="flex gap-1 flex-wrap text-xs">
              <span v-for="(v, k) in scoreResult.triggers_actifs" :key="k" class="bg-red-100 text-red-700 px-2 py-0.5 rounded">{{ k }} actif</span>
            </div>
            <div class="grid grid-cols-5 gap-2 pt-1">
              <div v-for="(v, k) in scoreResult.axes" :key="k" class="text-center">
                <div class="text-xs text-gray-500">Axe {{ k }}</div>
                <div :class="v === 2 ? 'text-red-600' : v === 1 ? 'text-orange-500' : 'text-green-600'" class="font-bold text-sm">{{ v }}/2</div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- Navigation -->
      <div class="flex items-center justify-between">
        <button v-if="currentStep > 0" @click="currentStep--" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">← Précédent</button>
        <div v-else />
        <div class="flex gap-3">
          <button @click="save" :disabled="saving" class="px-5 py-2 border border-[#1a2e4a] text-[#1a2e4a] rounded-lg text-sm font-medium hover:bg-[#1a2e4a]/5 disabled:opacity-50 transition-colors">
            {{ saving ? 'Enregistrement…' : 'Enregistrer' }}
          </button>
          <button v-if="currentStep < steps.length - 1" @click="currentStep++" class="px-5 py-2 bg-[#1a2e4a] text-white rounded-lg text-sm font-medium hover:bg-[#1a2e4a]/90 transition-colors">
            Suivant →
          </button>
        </div>
      </div>
    </div>
  </AppLayout>
</template>

<script setup lang="ts">
import { ref, reactive, onMounted } from 'vue'
import { RouterLink, useRoute } from 'vue-router'
import axios from 'axios'
import AppLayout from '@/layouts/AppLayout.vue'
import { useAuthStore } from '@/stores/auth'

const route = useRoute()
const auth = useAuthStore()
const dossierId = route.params.id as string

const dossierRef = ref('')
const currentStep = ref(0)
const saving = ref(false)
const saveError = ref('')
const saveOk = ref(false)
const hasMandataire = ref(false)

const steps = ['Identité PM', 'Coordonnées', 'Infos PM', 'Mandataire', 'BE & Actionnaires', 'PPE & Opération', 'Origine fonds']

const form = reactive({
  relation_type: 'initiale',
  denomination_sociale: '',
  forme_juridique: '',
  nom_representant_legal: '',
  numero_rccm: '',
  numero_contribuable: '',
  libelle_activite: '',
  adresse: '',
  telephone: '',
  whatsapp: '',
  email: '',
  mandataire: { prenom_nom: '', type_piece: '', numero_piece: '', date_naissance: '', nationalite: '', pays_residence: '', fonction: '' },
  ppe_detectee: false,
  ppe_detail: '',
  operations_cochees: { achat_immo: false, manipulation_fonds: false, creation_societe: false, fiducicommis: false, succession: false, donation: false, autre_detail: '' } as Record<string, any>,
  description_operation: '',
  origine_fonds: { activite: false, associes: false, vente_immeuble: false, bancaire: false, autres: '', propriete_intervenants: false, propriete_tiers: false, interet_tiers: false, territoire_ivoirien: true, pays_provenance: '' } as Record<string, any>,
  infos_pm: { domaine_activite: '', nature_pm: '', cotee: false, marche_reglemente: '' } as Record<string, any>,
  anciennete_pro: '',
  date_signature: '',
})

const beList = ref<Array<{ raison_sociale_nom: string; cni_passeport: string; pourcentage: number | null; nationalite: string; pays_residence: string }>>([])
const actList = ref<Array<{ raison_sociale_nom: string; cni_passeport: string; pourcentage: number; pays_residence: string; ordre: number }>>([])

interface ScoreResult { score: number; classification: string; triggers_actifs: Record<string, boolean>; force_par_trigger: boolean; trigger_principal: string | null; axes: Record<string, number> }
const scoreResult = ref<ScoreResult | null>(null)

const operationOptions = [
  { key: 'succession', label: 'Succession / Liquidation' },
  { key: 'donation', label: 'Donation' },
  { key: 'creation_societe', label: 'Constitution / Modification de société' },
  { key: 'achat_immo', label: 'Acquisition immobilière' },
  { key: 'manipulation_fonds', label: 'Manipulation de fonds' },
  { key: 'fiducicommis', label: 'Fiducicommis' },
]

const fondOptions = [
  { key: 'activite', label: "Revenus d'exploitation" },
  { key: 'associes', label: "Apports d'associés / actionnaires" },
  { key: 'vente_immeuble', label: "Cession d'actifs" },
  { key: 'bancaire', label: 'Crédit bancaire' },
  { key: 'propriete_intervenants', label: 'Propriété des intervenants' },
  { key: 'propriete_tiers', label: "Propriété d'un tiers" },
  { key: 'interet_tiers', label: "Agit pour le compte d'un tiers" },
  { key: 'territoire_ivoirien', label: 'Fonds sur le territoire ivoirien' },
]

function headers() { return auth.accessToken ? { Authorization: `Bearer ${auth.accessToken}` } : {} }
function classBadge(c: string) {
  return c === 'ELEVE' ? 'bg-red-100 text-red-700' : c === 'MOYEN' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'
}
function addBE() { beList.value.push({ raison_sociale_nom: '', cni_passeport: '', pourcentage: null, nationalite: '', pays_residence: '' }) }
function addActionnaire() { actList.value.push({ raison_sociale_nom: '', cni_passeport: '', pourcentage: 0, pays_residence: '', ordre: actList.value.length + 1 }) }

function buildPayload() {
  const d: any = { ...form }
  d.mandataire = { ...form.mandataire }
  d.operations_cochees = { ...form.operations_cochees }
  d.origine_fonds = { ...form.origine_fonds }
  d.infos_pm = { ...form.infos_pm }
  if (!d.date_signature) d.date_signature = null
  if (!d.anciennete_pro) d.anciennete_pro = null
  if (!d.forme_juridique) d.forme_juridique = null
  return d
}

async function save() {
  saving.value = true; saveError.value = ''; saveOk.value = false
  try {
    await axios.put(`/api/dossiers/${dossierId}/kyc/pm`, buildPayload(), { headers: headers() })

    for (const be of beList.value) {
      if (be.raison_sociale_nom) {
        await axios.post(`/api/dossiers/${dossierId}/kyc/pm/be`, be, { headers: headers() })
      }
    }
    beList.value = []

    for (let i = 0; i < actList.value.length; i++) {
      const act = actList.value[i]
      if (act.raison_sociale_nom) {
        await axios.post(`/api/dossiers/${dossierId}/kyc/pm/actionnaires`, { ...act, ordre: i + 1 }, { headers: headers() })
      }
    }
    actList.value = []

    const { data: s } = await axios.get(`/api/dossiers/${dossierId}/kyc/pp/score`, { headers: headers() }).catch(
      () => axios.get(`/api/dossiers/${dossierId}/kyc/pm`, { headers: headers() })
    )
    // Try to get fresh score from dossier
    const { data: dossier } = await axios.get(`/api/dossiers/${dossierId}`, { headers: headers() })
    if (dossier.classification) {
      scoreResult.value = { score: dossier.score_base, classification: dossier.classification, triggers_actifs: {}, force_par_trigger: dossier.force_par_trigger, trigger_principal: dossier.trigger_actif, axes: {} }
    }
    saveOk.value = true
    setTimeout(() => { saveOk.value = false }, 3000)
  } catch (e: any) {
    saveError.value = e.response?.data?.detail ?? "Erreur lors de l'enregistrement."
  } finally { saving.value = false }
}

async function loadExisting() {
  try {
    const { data: d } = await axios.get(`/api/dossiers/${dossierId}`, { headers: headers() })
    dossierRef.value = d.reference
    if (d.classification) {
      scoreResult.value = { score: d.score_base, classification: d.classification, triggers_actifs: {}, force_par_trigger: d.force_par_trigger, trigger_principal: d.trigger_actif, axes: {} }
    }
  } catch {}
  try {
    const { data: kyc } = await axios.get(`/api/dossiers/${dossierId}/kyc/pm`, { headers: headers() })
    const fields = ['relation_type','denomination_sociale','forme_juridique','nom_representant_legal',
      'numero_rccm','numero_contribuable','libelle_activite','adresse','telephone','whatsapp','email',
      'ppe_detectee','ppe_detail','description_operation','anciennete_pro','date_signature']
    for (const f of fields) if (kyc[f] !== undefined && kyc[f] !== null) (form as any)[f] = kyc[f]
    if (kyc.mandataire) { hasMandataire.value = true; Object.assign(form.mandataire, kyc.mandataire) }
    if (kyc.operations_cochees) Object.assign(form.operations_cochees, kyc.operations_cochees)
    if (kyc.origine_fonds) Object.assign(form.origine_fonds, kyc.origine_fonds)
    if (kyc.infos_pm) Object.assign(form.infos_pm, kyc.infos_pm)
    if (kyc.beneficiaires_effectifs?.length) {
      beList.value = kyc.beneficiaires_effectifs.map((b: any) => ({
        raison_sociale_nom: b.raison_sociale_nom, cni_passeport: b.cni_passeport ?? '',
        pourcentage: b.pourcentage, nationalite: b.nationalite ?? '', pays_residence: b.pays_residence ?? ''
      }))
    }
    if (kyc.actionnaires?.length) {
      actList.value = kyc.actionnaires.map((a: any) => ({
        raison_sociale_nom: a.raison_sociale_nom, cni_passeport: a.cni_passeport ?? '',
        pourcentage: a.pourcentage, pays_residence: a.pays_residence ?? '', ordre: a.ordre
      }))
    }
  } catch {}
}

onMounted(loadExisting)
</script>

<style scoped>
.section-title { @apply text-sm font-semibold text-gray-700 uppercase tracking-wide border-b border-gray-100 pb-2; }
.label { @apply block text-xs font-medium text-gray-700 mb-1; }
.req { @apply text-red-500; }
.input { @apply w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/30 focus:border-[#1a2e4a]; }
</style>
