<template>
  <div class="scoring-panel">
    ...
    <div v-if="currentScore !== null" class="score-banner" :class="`score-banner--${(currentNiveau ?? '').toLowerCase()}`">
      ...
      <button class="btn-recalc" @click="toggleAndRefresh">{{ showForm ? 'Masquer' : 'Recalculer' }}</button>
    </div>
    ...
    <div v-if="showForm" class="scoring-form">
      <div class="form-intro">
        <p class="form-intro-text">
          Matrice CDC Module 2 — 10 axes × 3 niveaux (0–20). Les axes 🤖 sont pré-calculés depuis le
          dossier/KYC ; toute baisse exige une justification (audit).
        </p>
      ...
      <!-- Données opération (axes 4 & 5 + trigger T2) -->
      <div class="op-fields">
        <label class="op-field">
          <span class="op-label">Montant de la transaction (FCFA)</span>
          <input v-model.number="op.montant_transaction" type="number" min="0" class="op-input" placeholder="0" />
        </label>
        <label class="op-field">
          <span class="op-label">Mode de paiement</span>
          <select v-model="op.mode_paiement" class="op-input">
        ...
        <label class="op-field">
          <span class="op-label">Nombre de parties</span>
          <input v-model.number="op.nb_parties" type="number" min="1" class="op-input" />
        </label>
      </div>
        ...
        <div v-for="axe in AXES" :key="axe.code" class="axe-row" :class="{ 'axe-row--auto': isAxeAuto(axe.code) }">
          <div class="axe-label-wrap">
            <span class="axe-label">
              {{ axe.label }}
              <span v-if="isAxeAuto(axe.code)" class="axe-auto-badge" title="Calculé automatiquement">🤖</span>
            ...
            <span v-if="isAxeAuto(axe.code) && prefillSource(axe.code)" class="axe-hint axe-hint--auto">{{ prefillSource(axe.code) }}</span>
            <span v-else-if="axe.hint" class="axe-hint">{{ axe.hint }}</span>
          </div>
          <div class="axe-options">
            <button
              v-for="opt in options" :key="opt.value" type="button" class="axe-btn"
      ...
      <!-- Triggers absolutoires -->
      <div class="triggers-box">
        <p class="triggers-title">Triggers absolutoires (forcent ÉLEVÉ — CDC §2.2)</p>
        <div class="trigger-derived">
          <span class="trigger-tag" :class="{ 'trigger-tag--on': t1Ppe }">T1 · PPE {{ t1Ppe ? '— actif' : '' }}</span>
          <span class="trigger-tag" :class="{ 'trigger-tag--on': t2Especes }">T2 · Espèces &gt; 15M {{ t2Especes ? '— actif' : '' }}</span>
        </div>
        <label class="trigger-check"><input v-model="flags.sur_liste_sanctions" type="checkbox" /> T3 · Client sur liste de sanctions (OFAC / UE-CSNU / GIABA-BCEAO)</label>
        <label class="trigger-check"><input v-model="flags.pays_liste_noire_gafi" type="checkbox" /> T4 · Pays liste noire GAFI</label>
        <label class="trigger-check"><input v-model="flags.pays_liste_grise_gafi" type="checkbox" /> T4 · Pays liste grise GAFI</label>
        <label class="trigger-check"><input v-model="flags.refus_documents" type="checkbox" /> T5 · Refus documentaire</label>
        <label class="trigger-check"><input v-model="flags.be_non_identifiable" type="checkbox" /> T6 · Bénéficiaire effectif non identifiable</label>
      </div>
      ...
      <div v-if="hasAbsolutoire" class="trigger-notice">
        ...
        Trigger <strong>{{ activeTriggers.join(', ') }}</strong> actif — niveau forcé à ÉLEVÉ indépendamment du score.
      </div>

      <!-- Total -->
      <div class="total-preview">
        <span class="total-label">Score total estimé</span>
        <span class="total-value" :class="`total-value--${previewNiveau.toLowerCase()}`">{{ previewTotal }}/20 — {{ previewNiveau }}</span>
      </div>

      <div class="form-actions">
        <button class="btn-cancel" type="button" @click="resetForm">Réinitialiser</button>
        <button class="btn-calculate" type="button" :disabled="saving" @click="calculate">
          <span v-if="saving" class="spinner" />{{ saving ? 'Calcul en cours…' : 'Calculer et enregistrer' }}
        </button>
      </div>
  ...
  <!-- Modale justification override (CDC §9.2) -->
  <Teleport to="body">
    <div v-if="overrideModal.open" class="modal-overlay" @click.self="cancelOverride">
      <div class="modal-box">
        ...
        <p class="modal-desc">Une justification est obligatoire (min. 50 caractères).</p>
        <textarea v-model="overrideModal.justification" class="modal-textarea" placeholder="Justification de la surcharge…" rows="4" />
        ...
        <div class="modal-actions">
          ...
          <button class="btn-modal-confirm" :disabled="overrideModal.justification.length < 50" @click="confirmOverride">Confirmer la surcharge</button>
        </div>
      </div>
    </div>
  </Teleport>
...
import {
  dossiersService,
  type DossierOut, type ScoringPrefill, type ModePaiement, type ScoreResult,
...
const currentScore = computed(() => props.dossier.score_base ?? null)
const currentNiveau = computed(() => props.dossier.classification ?? null)

// 10 axes canoniques (ordre CDC v4 §Module 2)
const AXES = [
  { code: 'type_client',     label: 'Type de client',           hint: 'Particulier / PM / structure complexe' },
  { code: 'pays_geographie', label: 'Pays / Géographie',        hint: 'CI-UEMOA / pays régulé / pays à risque' },
  { code: 'type_operation',  label: "Nature de l'opération",    hint: 'Vente / succession / montage complexe' },
  { code: 'montant',         label: 'Montant de la transaction', hint: '< 5M / 5–15M / > 15M FCFA' },
  { code: 'mode_paiement',   label: 'Mode de paiement',         hint: 'Virement / chèque / espèces' },
  { code: 'complexite',      label: 'Complexité juridique',     hint: 'Acte simple / 2–3 parties / complexe' },
  { code: 'ppe',             label: 'Personne politiquement exposée', hint: 'Non PPE / PPE détecté' },
  { code: 'coherence_doc',   label: 'Cohérence documentaire',   hint: 'Complet / doute / incohérent' },
  { code: 'secteur',         label: "Secteur d'activité",       hint: 'Classique / commerce / sensible' },
  { code: 'intermediaires',  label: 'Intermédiaires',           hint: 'Aucun / identifié / non clair' },
] as const
...
function defaultForm(): FormState {
  ...
  for (const axe of AXES) f[axe.code] = 0
  return f as FormState
}

const form = reactive<FormState>(defaultForm())
const op = reactive<{ montant_transaction: number | null; mode_paiement: ModePaiement | null; nb_parties: number }>({
  montant_transaction: props.dossier.montant_transaction ?? null,
  mode_paiement: (props.dossier.mode_paiement as ModePaiement | null) ?? null,
  nb_parties: props.dossier.nb_parties ?? 1,
})
const flags = reactive({
  sur_liste_sanctions: false,
  pays_liste_noire_gafi: false,
  pays_liste_grise_gafi: false,
  refus_documents: false,
  be_non_identifiable: false,
})
...
const overrideModal = reactive({
  open: false, axeCode: '', axeLabel: '', autoValue: 0, newValue: 0, justification: '', error: '',
})
...
function handleAxeClick(code: AxeCode, newValue: number) {
  const axe = AXES.find(a => a.code === code)
  if (!axe) return
  if (!isAxeAuto(code)) { form[code] = newValue; return }
  const autoValue = prefill.value?.axes[code]?.valeur ?? 0
  if (newValue === form[code]) return
...
function cancelOverride() { overrideModal.open = false }
function confirmOverride() {
  ...
  form[overrideModal.axeCode as AxeCode] = overrideModal.newValue
...
async function loadPrefill() {
  ...
  try {
    ...
    for (const [code, axe] of Object.entries(prefill.value.axes)) {
      if (code in form && !overrides[code]) form[code as AxeCode] = axe.valeur
    }
  } catch {
    // prefill non critique
  } finally {
...
onMounted(() => {
  if (currentScore.value !== null) showForm.value = false
  loadPrefill()
})
...
// ── Triggers (preview) ──────────────────────────────────────────────────────
const t1Ppe = computed(() => (prefill.value?.axes.ppe?.valeur ?? form.ppe) === 2)
const t2Especes = computed(() => op.mode_paiement === 'especes' && (op.montant_transaction ?? 0) > 15_000_000)
const activeTriggers = computed(() => {
  const t: string[] = []
  if (t1Ppe.value) t.push('T1')
  if (t2Especes.value) t.push('T2')
  if (flags.sur_liste_sanctions) t.push('T3')
  if (flags.pays_liste_noire_gafi || flags.pays_liste_grise_gafi) t.push('T4')
  if (flags.refus_documents) t.push('T5')
  if (flags.be_non_identifiable) t.push('T6')
  return t
})
const hasAbsolutoire = computed(() => activeTriggers.value.length > 0)

const previewTotal = computed(() =>
  Math.min(20, Math.max(0, (Object.values(form) as number[]).reduce((a, b) => a + b, 0))),
)
...
async function calculate() {
  ...
  try {
    const overridesList = Object.entries(overrides).map(([axe, justification]) => ({
      axe, valeur_override: form[axe as AxeCode], justification,
    }))
    const result = await dossiersService.calculateScore(props.dossier.id, {
      axes: { ...form },
      montant_transaction: op.montant_transaction,
      mode_paiement: op.mode_paiement,
      nb_parties: op.nb_parties,
      ...flags,
...
.score-banner { display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.25rem; border-radius: 10px; border: 1.5px solid; }
...
.btn-recalc { padding: 0.375rem 0.875rem; border: 1px solid var(--color-border); border-radius: 7px; background: var(--color-bg-card); font-size: 0.8125rem; cursor: pointer; color: var(--color-text-secondary); }
.btn-recalc:hover { border-color: var(--color-sidebar-bg); color: var(--color-sidebar-bg); }

.no-score-notice { display: flex; align-items: center; gap: 0.75rem; padding: 1rem 1.25rem; background: var(--color-bg-page); border-radius: 8px; border: 1px dashed var(--color-border); color: var(--color-text-muted); font-size: 0.875rem; }
...
.prefill-loading { font-size: 0.8125rem; color: var(--color-text-muted); padding: 0.5rem 0.75rem; background: var(--color-bg-page); border-radius: 6px; border: 1px solid var(--color-border); animation: pulse 1.2s ease-in-out infinite; }
@keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }

.op-fields { display: grid; grid-template-columns: 1fr 1fr 0.6fr; gap: 0.75rem; }
.op-field { display: flex; flex-direction: column; gap: 0.25rem; }
.op-label { font-size: 0.75rem; font-weight: 600; color: var(--color-text-secondary); }
.op-input { padding: 0.5rem; border: 1.5px solid var(--color-border); border-radius: 7px; font-size: 0.8125rem; background: var(--color-bg-card); color: var(--color-text-primary); }
.op-input:focus { outline: none; border-color: var(--color-sidebar-bg); }

.axes-grid { display: flex; flex-direction: column; gap: 0; border: 1px solid var(--color-border); border-radius: 8px; overflow: hidden; }
.axe-row { display: grid; grid-template-columns: 1fr auto auto; align-items: center; gap: 0.75rem; padding: 0.625rem 1rem; border-bottom: 1px solid var(--color-border); }
...
.axe-btn { padding: 4px 10px; border-radius: 5px; border: 1.5px solid var(--color-border); background: var(--color-bg-card); font-size: 0.75rem; font-weight: 500; cursor: pointer; color: var(--color-text-secondary); transition: all 0.1s; }
...
.triggers-box { display: flex; flex-direction: column; gap: 0.4rem; padding: 0.75rem 1rem; border: 1px solid var(--color-border); border-radius: 8px; background: var(--color-bg-page); }
.triggers-title { font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; color: var(--color-text-secondary); margin: 0 0 0.25rem; }
.trigger-derived { display: flex; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 0.25rem; }
.trigger-tag { padding: 2px 8px; border-radius: 5px; font-size: 0.6875rem; font-weight: 700; background: var(--color-bg-card); border: 1px solid var(--color-border); color: var(--color-text-muted); }
.trigger-tag--on { background: var(--color-risk-high-bg); border-color: var(--color-risk-high); color: var(--color-risk-high); }
.trigger-check { display: flex; align-items: center; gap: 0.5rem; font-size: 0.8125rem; color: var(--color-text-secondary); cursor: pointer; }

.trigger-notice { display: flex; align-items: center; gap: 0.5rem; padding: 0.625rem 0.875rem; background: var(--color-risk-high-bg); border: 1px solid var(--color-risk-high); border-radius: 7px; font-size: 0.8125rem; color: var(--color-risk-high); font-weight: 500; }
.trigger-icon { width: 16px; height: 16px; flex-shrink: 0; }

.total-preview { display: flex; align-items: center; justify-content: space-between; padding: 0.75rem 1rem; background: var(--color-bg-page); border: 1px solid var(--color-border); border-radius: 8px; }
...
.btn-cancel { padding: 0.5rem 1rem; border: 1px solid var(--color-border); border-radius: 8px; background: none; font-size: 0.8125rem; cursor: pointer; color: var(--color-text-secondary); }
.btn-cancel:hover { background: var(--color-bg-page); }
.btn-calculate { display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1.25rem; background: var(--color-sidebar-bg); color: #fff; border: none; border-radius: 8px; font-size: 0.875rem; font-weight: 600; cursor: pointer; }
...
.modal-box { background: var(--color-bg-card); border-radius: 12px; padding: 1.5rem; width: 100%; max-width: 480px; box-shadow: 0 20px 60px rgba(0,0,0,0.25); }
...
.modal-textarea { width: 100%; box-sizing: border-box; padding: 0.625rem; border: 1.5px solid var(--color-border); border-radius: 8px; font-size: 0.8125rem; resize: vertical; min-height: 80px; color: var(--color-text-primary); background: var(--color-bg-page); }
...
.btn-modal-cancel { padding: 0.5rem 1rem; border: 1px solid var(--color-border); border-radius: 8px; background: none; cursor: pointer; font-size: 0.8125rem; color: var(--color-text-secondary); }
.btn-modal-confirm { padding: 0.5rem 1.25rem; background: var(--color-sidebar-bg); color: #fff; border: none; border-radius: 8px; font-size: 0.875rem; font-weight: 600; cursor: pointer; }
